...
SampledQMax::SampledQMax(int q, float gamma){
...
void SampledQMax::maintenance(){
	_phi = findKthLargestAndPivot();
}

int* SampledQMax::largestQ(){
	maintenance();
	return _A + _nminusq;
}
...
int SampledQMax::PartitionAroundPivot(int left, int right, int pivot_idx, int* nums) {
...
uint32_t SampledQMax::mm256_extract_epi32_var_indx(int i){   
...
int SampledQMax::GenerateRandom(int max){
//     return std::rand() % max; 
    int indx = 0;
    if(counter >= 8){
        rand_bits =_mm256_set_epi64x(gen_arr(), gen_arr(), gen_arr(), gen_arr());
...
int SampledQMax::findValueIndex(int value){
    for(int i=0;i<_actualsize;i++){
        if(_A[i]==value){
            return i;
        }
...
int SampledQMax::checkPivot(int value){
...
int SampledQMax::findKthLargestAndPivot(){
    int tries=2;
    while(tries!=0){
