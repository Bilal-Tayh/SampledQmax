...
void benchmark_qmax_LV_V2(int** data, ofstream& ostream, int q, int actualSize) {
    int* elements = *data;
    struct timeb begintb, endtb;
    clock_t begint, endt;
    double time;
    SampledQMax_LV_V2 qmax(q,actualSize);
    begint = clock();
    ftime(&begintb);
    for (int i = 0; i < N; i++) {
        qmax.insert(elements[i]);
...
//          
        ...
        benchmark_qmax_LV_V2(&data, ostream, 1000000,(int) (1000000*(1+4)));
