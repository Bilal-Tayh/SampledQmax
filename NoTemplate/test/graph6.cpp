...
void benchmark_hhqmax_LV(key** keys, ofstream &ostream, string dataset, int numKeys, int q, int actualSize) {
  key *elements = *keys;
  struct timeb begintb, endtb;
  clock_t begint, endt;
  double time;
...
int main() {
  ...
  for (int run = 0; run < 2; run++) {
    ...
    for (auto& stream : streams) {
        ...
        benchmark_hhqmax_LV(&k, *stream, dataset, size, 1000000,(int) (1000000*(1+0.25)));
