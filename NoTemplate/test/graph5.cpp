...
void benchmark_psqmax_LV(key** keys, val** vals, ofstream &ostream, string dataset, int numKeys, int q, int actualSize) {
  std::random_device _rd;
  std::mt19937 _e2(_rd());
  std::uniform_real_distribution<double> _dist(0,1);
  key *elements = *keys;
  ...
  clock_t begint, endt;
  double time;
  SampledQMaxKV_LV_V2 lvqmax = SampledQMaxKV_LV_V2(q,actualSize);
  begint = clock();
  ftime(&begintb);
  for (int i = 0; i < numKeys; i++) {
    val priority = weights[i] / (1-_dist(_e2));
...
int main() {
  ...
  for (int run = 0; run < 1; run++) {
      ...
      for (int run = 0; run < 1; run++) {
        ...
        benchmark_psqmax_LV(&k, &v, *stream, dataset, size, 1000000,(int) (1000000*(1+0.25)));
